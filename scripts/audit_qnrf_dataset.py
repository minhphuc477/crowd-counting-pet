...
def inspect_split(root: Path, split_name: str, expected_max_side: int) -> dict:
    ...
    for image_path in images:
        ...
        outside_xy += int(((x < 0) | (x >= width) | (y < 0) | (y >= height)).sum())
    ...
    if outside_xy:
        raise ValueError(
            f"{split_name}: {outside_xy} annPoints coordinates are outside image bounds "
            f"when interpreted as x/y (outside_if_swapped={outside_if_swapped})"
        )
    if oversized:
        first = oversized[0]
        raise ValueError(
...
def main() -> int:
    ...
    report = {
        ...
        "splits": {
            split: inspect_split(root, split, args.expected_max_side)
