...
def run_eval(item, args, log_path):
    ...
    if args.eval_tile_trigger_count is not None:
        cmd.extend(['--eval_tile_trigger_count', str(args.eval_tile_trigger_count)])
    if args.eval_tile_trigger_area is not None:
        cmd.extend(['--eval_tile_trigger_area', str(args.eval_tile_trigger_area)])
...
def parse_args():
    ...
    parser.add_argument('--eval_count_source', default='', choices=('', 'pet', 'count_head', 'zip', 'zip_pet_blend', 'zip_tail_blend', 'measure'))
