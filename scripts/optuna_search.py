...
def objective(trial, args):
    ...
    lr_backbone = trial.suggest_loguniform('lr_backbone', 1e-6, 1e-4)
    ...
    cmd = [sys.executable, 'main.py', '--backbone', args.backbone, '--seed', '42', '--output_dir', str(out_dir)] + shlex.split(extra)
    print('Running trial', trial.number, 'cmd:', ' '.join(cmd))
    r = subprocess.run(cmd)
    if r.returncode != 0:
        raise optuna.exceptions.TrialPruned()
    mae = read_mae(args.dataset_file, out_dir)
    if mae is None:
        raise optuna.exceptions.TrialPruned()
    return mae
