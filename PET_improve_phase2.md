...
**Why it works for crowd counting specifically:** The SHA test set has 182 images with perspective distortion — some images are taken from elevated angles, making left/right asymmetric. Averaging predictions on the original image, its horizontal flip, and its vertical flip reduces per-image variance without any retraining. Since you are counting (a scalar per image), averaging 3–5 predictions is trivially cheap.

**How — use these at eval time:**
...
For point-based models like PET, TTA is applied as:

1. Run forward pass on original image → count_1
2. Run forward pass on H-flipped image → count_2
3. Run forward pass on V-flipped image → count_3
4. Final count = (count_1 + count_2 + count_3) / 3
...
## Phase 6: Replace the Hungarian Matching Loss (Deepest Structural Fix)

**What:** PET uses bipartite matching (Hungarian algorithm) to assign predicted point queries to GT points during training. This assignment changes every epoch as the model improves, creating unstable gradients especially early in training. There are two validated replacements from the literature:

### Option A — Optimal Transport Loss (OT-Loss)

Instead of hard 1-to-1 Hungarian assignment, OT finds a soft assignment distribution between predictions and GT points that minimizes transport cost. This gives every prediction a gradient signal, not just the matched ones. OT is differentiable and produces smoother loss landscapes on small datasets like SHA.

From the DM-Count paper (NeurIPS 2020): OT loss on SHA-A with a VGG backbone gives MAE ~59. Combined with a stronger backbone and point-based formulation (vs density maps), this drops further. The key advantage for PET: OT assigns partial credit to near-miss predictions, so the model learns faster early in training when the Hungarian matcher would otherwise assign random matches.

```python
# Add to your losses/matching.py
import torch
from scipy.optimize import linear_sum_assignment

def ot_loss(pred_points, pred_scores, gt_points, epsilon=0.1, num_iters=100):
    """
    Sinkhorn OT loss between predictions and GT points.
    epsilon: regularization strength (higher = smoother assignment)
    """
    # Cost matrix: pairwise L2 distances
    C = torch.cdist(pred_points, gt_points, p=2)  # [N_pred, N_gt]
  
    # Normalize predictions by score
    a = pred_scores / pred_scores.sum()  # predicted distribution
    b = torch.ones(len(gt_points), device=gt_points.device) / len(gt_points)  # uniform GT
...
### Option B — Bayesian Loss (Simpler, Proven)

Bayesian Loss (ICCV 2019, Ma et al.) constructs a density contribution probability model from point annotations. Instead of matching predictions to GT points one-to-one, it computes the expected count contribution at each GT point location from the full predicted density. This is robust to annotation noise and perspective effects — both major issues in SHA.

For PET's point-based formulation, Bayesian Loss is adapted as: for each GT point, compute the expected number of predicted points within a radius r weighted by their classification scores. The loss penalizes when this expectation deviates from 1.

```python
def bayesian_loss(pred_points, pred_scores, gt_points, sigma=8.0):
    """
    Bayesian loss for point-based crowd counting.
    For each GT point, the expected count within radius sigma should be 1.
    """
    # Pairwise distances: [N_gt, N_pred]
    dists = torch.cdist(gt_points.float(), pred_points.float(), p=2)
  
    # Gaussian kernel weights
    weights = torch.exp(-dists**2 / (2 * sigma**2))  # [N_gt, N_pred]
  
    # Expected count at each GT point = sum of weighted scores
    expected = (weights * pred_scores.unsqueeze(0)).sum(dim=1)  # [N_gt]
  
    # Each GT point should have expected count of 1
    loss = torch.nn.functional.mse_loss(expected, torch.ones_like(expected))
    return loss
```
...
## Phase 7: What STEERER Does That PET Does Not — Scale Inheritance

**Context from the literature:** STEERER (ICCV 2023, same conference as PET) achieves competitive MAE on SHA through a fundamentally different approach to scale variation. Instead of the quadtree split map, STEERER uses a Feature Selection and Inheritance Adaptor (FSIA) that selectively forwards scale-customized features at each scale level. The key insight: instead of deciding "dense or sparse" as a binary routing decision, STEERER learns which feature channels are most useful for each density scale and inherits them across resolutions.

**What this means for your repo:** The quadtree split map is PET's version of this idea — routing to dense branch = use fine-grained features, routing to sparse branch = use coarse features. The weakness is it's a hard binary decision. A soft version — letting each spatial location use a weighted combination of scale features — would be strictly more powerful.

**Concrete implementation (soft split):** Instead of the binary split map sigmoid that routes to either branch, use a soft gating:

```python
# ORIGINAL hard split:
split_map = torch.sigmoid(self.split_head(features))  # [B, 1, H, W]
# Use split_map > 0.5 to decide routing → hard binary

# SOFT split (more powerful):
gate = torch.sigmoid(self.split_head(features))  # [B, 1, H, W], still in [0,1]
# Interpolate between sparse and dense features:
fused_features = gate * dense_features + (1 - gate) * sparse_features
# No hard threshold — every location gets a mix
```

This removes the hard threshold entirely. The model learns smooth transitions between dense and sparse regions instead of discontinuous routing decisions. This directly addresses the instability seen in your diagnostic (img 06: gt=1175, pred=1390 — a hard routing error for an extreme-density image).

**Expected gain:** 1–2 MAE. This is a model architecture change and requires retraining from scratch.

---

## Complete Priority Order (Everything Together)

Work through these strictly in order. Do not skip phases. Each phase must be validated (check `eval_history.jsonl` at epoch 300) before starting the next.

| Priority | Phase                 | Change                               | Cost           | Expected gain |
| -------- | --------------------- | ------------------------------------ | -------------- | ------------- |
| 1        | Split supervision fix | `--quadtree_loss_coef 0.5`         | 0 code changes | 1–2 MAE      |
