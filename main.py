...
MODEL_RECIPES['vgg_apglc_measure_qnrf'] = {
    **MODEL_RECIPES['vgg_apglc'],
    ...
    'measure_loss_count_coef': 0.1,
    'measure_loss_transport_coef': 0.0,
    'measure_loss_start_epoch': 0,
    'measure_loss_end_epoch': -1,
    'measure_loss_warmup_epochs': 100,
    'measure_loss_feature_grad_scale': 0.05,
    'measure_loss_feature_grad_start_epoch': 100,
    'measure_loss_feature_grad_warmup_epochs': 200,
...
# Its only difference from qnrf_tail_scale_rifi is the required, count-preserving
# train-annotation manifest validated before datasets are built.  Keeping that
# contract in a named recipe prevents restored labels from being silently mixed
# with an unrelated split or used to score the holdout/test annotations.
MODEL_RECIPES['vgg_apglc_qnrf_sae_scale_rifi'] = {
    **MODEL_RECIPES['vgg_apglc_qnrf_tail_scale_rifi'],
}
...
def validate_annotation_override_contract(args, validation_protocol):
    """Reject annotation overrides that could alter evaluation or leak a holdout."""
    override_dir = getattr(args, 'annotation_override_dir', '')
    recipe_name = getattr(args, 'model_recipe', 'none')
    requires_qnrf_restoration = recipe_name == 'vgg_apglc_qnrf_sae_scale_rifi'
    if requires_qnrf_restoration and getattr(args, 'dataset_file', '') != 'QNRF':
        raise ValueError(f'model_recipe={recipe_name!r} is implemented only for QNRF')
    if requires_qnrf_restoration and not override_dir:
        raise ValueError(
            f'model_recipe={recipe_name!r} requires --annotation_override_dir '
            'generated by scripts/restore_qnrf_annotations.py'
        )
    if not override_dir:
        return

    if getattr(args, 'dataset_file', '') != 'QNRF':
        return

    manifest_path = Path(override_dir) / 'annotation_override_manifest.json'
    if not manifest_path.is_file():
        raise FileNotFoundError(
            f'QNRF annotation overrides require a manifest: {manifest_path}'
        )
    with manifest_path.open('r', encoding='utf-8') as handle:
        manifest = json.load(handle)
    if manifest.get('dataset') != 'QNRF':
        raise ValueError('QNRF annotation override manifest has the wrong dataset')
    if requires_qnrf_restoration and manifest.get('method') != 'shifted_annotation_restoration':
        raise ValueError(
            'QNRF annotation overrides must come from the shifted-annotation '
            'restoration pipeline'
        )
    if not bool(manifest.get('count_preserved', False)):
        raise ValueError('QNRF annotation overrides must preserve every image count')
    if validation_protocol == 'train_holdout':
        expected_fraction = float(getattr(args, 'train_holdout_fraction', 0.1))
        expected_seed = int(getattr(args, 'train_holdout_seed', args.seed))
        actual_fraction = float(manifest.get('holdout_fraction', -1.0))
        actual_seed = int(manifest.get('holdout_seed', -1))
        if not math.isclose(actual_fraction, expected_fraction, rel_tol=0.0, abs_tol=1e-12):
            raise ValueError(
                'QNRF restorer/detector holdout fractions differ: '
                f'manifest={actual_fraction}, detector={expected_fraction}'
            )
        if actual_seed != expected_seed:
            raise ValueError(
                'QNRF restorer/detector holdout seeds differ: '
                f'manifest={actual_seed}, detector={expected_seed}'
            )
...
def set_reproducibility(seed, deterministic=True):
    ...
    # Force deterministic CUDA algorithm selection (reduces GPU non-determinism
    # from atomics such as scatter_add in PET point queries). warn_only keeps
