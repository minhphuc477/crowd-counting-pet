"""
MaxViT backbone adapter for PET using timm.
"""
...
class BackboneBase_MaxViT(nn.Module):
    def __init__(self, backbone: nn.Module, num_channels: int):
        ...
        if len(self.stage_channels) < 4:
            raise ValueError(f'MaxViT backbone must expose at least 4 stages, got {len(self.stage_channels)}')
...
def build_backbone_maxvit(args):
    name = getattr(args, 'backbone', 'maxvit_small_tf_224')
    if 'maxvit' not in name:
        raise ValueError(f'Unsupported MaxViT backbone: {name}')
    position_embedding = build_position_encoding(args)
    backbone = Backbone_MaxViT(model_name=name, pretrained=True)
