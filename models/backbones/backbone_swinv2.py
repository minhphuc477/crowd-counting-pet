"""
Swin Transformer V2 backbone adapter for PET using timm.
"""
...
class BackboneBase_SwinV2(nn.Module):
    def __init__(self, backbone: nn.Module, num_channels: int):
        ...
        if len(self.stage_channels) < 4:
            raise ValueError(f'SwinV2 backbone must expose at least 4 stages, got {len(self.stage_channels)}')
        self.stage_channels = self.stage_channels[:4]
        self.stage_reductions = self.stage_reductions[:4]
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        features_fpn_4x = features_fpn[self.output_reduction_to_index[self.stage_reductions[0]]]
        # choose an 8x-like feature if available
        reduction_keys = list(self.output_reduction_to_index.keys())
        red8 = reduction_keys[-1]
        features_fpn_8x = features_fpn[self.output_reduction_to_index[red8]]
...
def build_backbone_swinv2(args):
    name = getattr(args, 'backbone', 'swinv2_base_window8_256')
    if 'swinv2' not in name:
        raise ValueError(f'Unsupported SwinV2 backbone: {name}')
    position_embedding = build_position_encoding(args)
    backbone = Backbone_SwinV2(model_name=name, pretrained=True)
