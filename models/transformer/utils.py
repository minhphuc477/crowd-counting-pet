...
def enc_win_partition(src, pos_embed, mask, enc_win_h, enc_win_w):
    ...
    if H % enc_win_h != 0 or W % enc_win_w != 0:
        raise ValueError(
            f'Feature shape {(H, W)} must be divisible by encoder window '
...
def enc_win_partition_reverse(windows, window_size_h, window_size_w, H, W):
    ...
    """
    windows_per_image = (H // window_size_h) * (W // window_size_w)
    if windows_per_image <= 0 or windows.shape[1] % windows_per_image != 0:
        raise ValueError(
            f'Cannot reverse {windows.shape[1]} windows into feature shape '
            f'{(H, W)} with window size {(window_size_h, window_size_w)}.'
        )
    B = windows.shape[1] // windows_per_image
...
def window_partition(x, window_size_h, window_size_w):
    ...
    if H % window_size_h != 0 or W % window_size_w != 0:
        raise ValueError(
            f'Input shape {(H, W)} must be divisible by window size '
            f'{(window_size_h, window_size_w)}.'
        )
